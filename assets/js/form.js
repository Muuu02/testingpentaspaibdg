...
function renderDynamicPesertaForm() {
    ...
    if (selectedLomba === 'lccp') {
        ...
        html += `
            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Regu <span class="text-red-500">*</span></label>
                <input type="text" name="namaRegu" required class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none text-gray-900" placeholder="Nama regu">
            </div>
        `;
        for (let i = 1; i <= 3; i++) {
    ...
    // LSQR - 9-11 orang homogen
        ...
        html += `
            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Grup <span class="text-red-500">*</span></label>
                <input type="text" name="namaGrup" required class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none text-gray-900" placeholder="Nama grup">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Gender Grup <span class="text-red-500">*</span></label>
                <select name="genderGrup" required class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none bg-white text-gray-900">
        ...
        html += `
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p class="text-sm text-yellow-800"><i class="fas fa-info-circle mr-2"></i>Minimal 9 anggota wajib diisi. Anggota 10-11 opsional.</p>
            </div>
        `;
    }
    ...
    // Perorangan dengan pilihan gender (LDC, MTQ, MHQ, LKI)
        ...
        html += `
            <div class="mb-4">
                ...
                <select name="jenisKelamin" required class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none bg-white text-gray-900">
        ...
        if (selectedLomba === 'mtq') {
            html += `
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Pilihan Maqro <span class="text-red-500">*</span></label>
                    <select name="maqro" required class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none bg-white text-gray-900">
    ...
    // LPA - 1 putra saja
        ...
        html += `
            <div class="mb-4">
                ...
                <input type="text" value="Putra" readonly class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl bg-gray-100 text-gray-600">
...
function generateAnggotaCard(index, label, showGender, isRequired, showPeran = false, isOptional = false) {
    ...
    const requiredMark = isRequired ? '<span class="text-red-500">*</span>' : '<span class="text-gray-400">(opsional)</span>';
    const genderValue = showGender ? '' : 'value="P" readonly';
    const genderClass = showGender ? '' : 'bg-gray-100 text-gray-600';
    ...
    if (showPeran) {
        ...
        if (isPutri) {
            peranHtml = `
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Peran ${requiredMark}</label>
                    <select name="peran${index}" ${requiredAttr} class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none bg-white text-gray-900">
    ...
    return `
        <div class="anggota-card bg-gray-50 rounded-xl p-4 md:p-6 border-2 border-gray-200 ${isOptional ? 'opacity-75' : ''}">
            <div class="flex items-center justify-between mb-4">
                <h4 class="font-bold text-emerald-700">${label}</h4>
                ${index > 1 && !showPeran ? `<button type="button" onclick="copyFromFirst(${index})" class="text-sm text-emerald-600 hover:text-emerald-800"><i class="fas fa-copy mr-1"></i>Copy dari Anggota 1</button>` : ''}
            </div>
            <div class="grid md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Lengkap ${requiredMark}</label>
                    <input type="text" name="nama${index}" ${requiredAttr} class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none text-gray-900" placeholder="Nama lengkap sesuai ijazah">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">NISN ${requiredMark}</label>
                    <input type="text" name="nisn${index}" ${requiredAttr} maxlength="10" class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none text-gray-900" placeholder="10 digit NISN">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal Lahir ${requiredMark}</label>
                    <input type="date" name="ttl${index}" ${requiredAttr} class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none text-gray-900">
                </div>
                ${showGender ? `
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Jenis Kelamin ${requiredMark}</label>
                    <select name="jk${index}" ${requiredAttr} class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none bg-white text-gray-900">
                        <option value="">Pilih</option>
                        <option value="L">Putra</option>
                        <option value="P">Putri</option>
                    </select>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Jenis Kelamin</label>
                    <input type="text" ${genderValue} readonly class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl ${genderClass}">
                    <input type="hidden" name="jk${index}" value="P">
                </div>
                `}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Kelas ${requiredMark}</label>
                    <select name="kelas${index}" ${requiredAttr} class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none bg-white text-gray-900">
                        <option value="">Pilih</option>
                ...
                ${peranHtml}
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Upload Foto ${requiredMark}</label>
                    <input type="file" name="foto${index}" ${requiredAttr} accept="image/*" class="input-field w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none text-gray-900 bg-white">
...
function renderResume() {
    ...
    if (selectedLomba === 'lccp' || selectedLomba === 'lpsb') {
        html += `<p class="font-semibold text-gray-900 mb-2">Nama Regu: ${formData.namaRegu || '-'}</p>`;
    }
    
    if (selectedLomba === 'lsqr') {
        html += `<p class="font-semibold text-gray-900 mb-2">Nama Grup: ${formData.namaGrup || '-'}</p>`;
        html += `<p class="text-sm text-gray-600 mb-3">Gender: ${formData.genderGrup === 'L' ? 'Putra' : 'Putri'}</p>`;
    }
    ...
    for (let i = 1; i <= pesertaCount; i++) {
        ...
        html += `
            <div class="flex items-center justify-between py-2 border-b last:border-0">
                <div>
                    <p class="font-semibold text-gray-900">${formData[`nama${i}`]}${peran}</p>
                    <p class="text-sm text-gray-600">NISN: ${formData[`nisn${i}`]} | ${jk} | Kelas ${formData[`kelas${i}`]}</p>
                </div>
            </div>
        `;
    }
    
    container.innerHTML = html || '<p class="text-gray-500">Tidak ada data peserta</p>';
}
